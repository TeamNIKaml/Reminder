package com.teamnikaml.mycredentials.password.database;
...
public class PasswordHelper implements IDBHelper {
	...
	public boolean onCreate() {
		...
		dbHelper = new DBHelper(dataSource.getContext(), 1,
				AppConstants.DB_NAME_PASSWORD, AppConstants.PASSWORD_DB_QUERY);
		return true;
	...
	private class DBTask extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... operation) {
			...
			if (operation[0].equalsIgnoreCase("insert")) {
				...
				database.insert(AppConstants.PASSWORD_TABLE_NAME, null,
				...
				SQLiteDatabase database = dbHelper.getWritableDatabase();
				database.update(AppConstants.PASSWORD_TABLE_NAME,
						dataSource.passwordToContentValues(),
				...
				SQLiteDatabase dataBase = dbHelper.getWritableDatabase();
				dataBase.delete(AppConstants.PASSWORD_TABLE_NAME,
						whereClause, whereArgs);
				System.out.println("where clause :"+whereClause+"\n where args :"+Arrays.toString(whereArgs));
				dataBase.close();
				...
				Cursor cursor = database.query(AppConstants.PASSWORD_TABLE_NAME,
						projection,
						whereClause, whereArgs,
						null, null, sortOrder);
