package com.teamnikaml.mycredentials.reminder.database;
	...
	private class DBTask extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... operation) {
			...
			if (operation[0].equalsIgnoreCase("insert")) {
				...
				database.insert(AppConstants.REMINDER_TABLE_NAME, null,
				...
				SQLiteDatabase database = dbHelper.getWritableDatabase();
				database.update(AppConstants.REMINDER_TABLE_NAME,
				...
				SQLiteDatabase dataBase = dbHelper.getWritableDatabase();
				dataBase.delete(AppConstants.REMINDER_TABLE_NAME, whereClause,
						whereArgs);
				...
				Cursor cursor = database.query(AppConstants.REMINDER_TABLE_NAME,
